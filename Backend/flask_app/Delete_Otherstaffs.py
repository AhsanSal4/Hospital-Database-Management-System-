...
def delete_staffs(s_id):
    ...
    try:
        ...
        cursor1.execute("SELECT Park_id FROM other_staffs WHERE s_id = %s", (s_id,))
        park_result = cursor1.fetchone()  # Fetch the result

        cursor1.execute("SELECT username FROM other_staffs WHERE s_id = %s", (s_id,))
        user_result = cursor1.fetchone() 

        cursor1.execute("DELETE FROM other_staffs WHERE s_id = %s", (s_id,))
        connection.commit()
        ...
        if cursor1.rowcount > 0:
            return jsonify({'message': 'Other_staff deleted successfully'})
        else:
            return jsonify({'error': 'Other_Staff not found'}), 404

    except Error as e:
        print(f"Error executing query: {e}")
        return jsonify({'error': 'Failed to delete Other_Staff '}), 500
